...
class ListenerDataset(Dataset):
    def __init__(self, split, data_dir, chain_file, utterances_file,
        ...
        # FOR THE DEMO, LOAD TEXT UTTERANCES
        # FOR HISTORY
        text_utt_file = self.split + '_text_utterances.pickle'
        with open(os.path.join(self.data_dir, text_utt_file), 'rb') as file:
            self.DEMO_chains = pickle.load(file)

        self.DEMO_utterances = dict()

        for tup in self.DEMO_chains:

            triple = tup[0], tup[1], tup[2]

            if triple in self.DEMO_utterances:
                print(triple)

            self.DEMO_utterances[triple] = self.DEMO_chains[tup]['utterance']
        ...
        for chain in self.chains[:self.subset_size]:
            ...
            for s in range(len(chain_utterances)):
                ...
                prev_chains_DEMO = defaultdict(list)
                    ...
                    if game_id in self.img2chain[im]:  # was there a linguistic chain for this image in this game
                        ...
                        if len(hist_utterances) > 0:
                            ...
                            for hu in [hist_utterances[-1]]:
                                ...
                                prev_chains_DEMO[im].extend(self.DEMO_utterances[hu])
                        else:
                ...
                self.data[len(self.data)] = {'utterance': cur_utterance_text,
                                             ...
                                             'DEMO_histories': prev_chains_DEMO
                                             }
    ...
    def get_collate_fn(device):
        ...
        def collate_fn(data):
            ...
            for key in batch.keys():
                if key not in ['prev_histories', 'DEMO_histories', 'utterance', 'representations',
