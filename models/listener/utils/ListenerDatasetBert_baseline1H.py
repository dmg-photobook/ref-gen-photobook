...
class ListenerDataset_1H(Dataset):
    def __init__(self, split, data_dir, chain_file, utterances_file,
        ...
        with open(os.path.join(data_dir, vectors_file), 'r') as file:
            self.image_features_orig = json.load(file)
        ...
        # WARNING, images should come in the same order in all 3 splits in the chains
        #  otherwise this would give different IDs to the same image
        #  in our case, 354 images exist in all the splits
        #  and the chains are sorted by images, so they are encountered in the same order in all 3 splits
        #  or sort them here!
        #  ALSO python now remembers insertion order in dictionaries

        one_hot_image_features = defaultdict()
        count_im = 0

        for img_key in self.img2chain:
            # print(img_key)
            one_hot_image_features[img_key] = [0.0]*(len(self.img2chain.keys()))
            one_hot_image_features[img_key][count_im] = 1.0

            count_im += 1

        self.image_features = one_hot_image_features
        self.img_dim = len(self.image_features)
