...
if __name__ == '__main__':
    ...
    model_file = 'all_histatt_TEST'

    with open('../FINAL_SPEAKERS/TEST_JSONS/histatt/hyps_hist_att_bert_test_2020-05-26-21-17-19.json', 'r') as f:
        hyps42 = json.load(f)

    with open('../FINAL_SPEAKERS/TEST_JSONS/histatt/hyps_hist_att_bert_test_2020-05-26-21-21-18.json', 'r') as f:
        hyps1 = json.load(f)

    with open('../FINAL_SPEAKERS/TEST_JSONS/histatt/hyps_hist_att_bert_test_2020-05-26-21-24-48.json', 'r') as f:
        hyps2 = json.load(f)

    with open('../FINAL_SPEAKERS/TEST_JSONS/histatt/hyps_hist_att_bert_test_2020-05-26-21-28-2.json', 'r') as f:
        hyps3 = json.load(f)

    with open('../FINAL_SPEAKERS/TEST_JSONS/histatt/hyps_hist_att_bert_test_2020-05-26-21-31-21.json', 'r') as f:
        hyps4 = json.load(f)

    with open('../FINAL_SPEAKERS/TEST_JSONS/histatt/refs_hist_att_bert_test_2020-05-26-21-17-19.json', 'r') as f:
        refs = json.load(f)

    with open('generated_outputs_model_' + model_file + '.txt', 'w') as f:
        ...
        for i, data in enumerate(test_loader):
            ...
            utt_string = [vocab.index2word[w.item()] for w in utterances_text_ids[0]] # single instance batch
            utt_string = ' '.join(utt_string)
            prev_string = [vocab.index2word[w.item()] for w in prev_utterance_ids[0]] # single instance batch
            prev_string = ' '.join(prev_string)
            ...
            f.write('I: ' + str(image_set[0]) + '\n')
            f.write('T: ' + str(image_set[0][targets]) + '\n') # single instance batch
            f.write('U: ' + utt_string + '\n')
            f.write('P: ' + prev_string + '\n')
            f.write('A: ' + actual_prev_string + '\n')
            f.write('R: ' + str(refs[i]) + '\n')
            f.write('H42: ' + str(hyps42[i]) + '\n')
            f.write('H1: ' + str(hyps1[i]) + '\n')
            f.write('H2: ' + str(hyps2[i]) + '\n')
            f.write('H3: ' + str(hyps3[i]) + '\n')
            f.write('H4: ' + str(hyps4[i]) + '\n' + '\n')

    # metric_dict = nlge.compute_metrics(refs, hyps)
    # print(metric_dict)
